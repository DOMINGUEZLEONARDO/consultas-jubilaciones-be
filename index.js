...
(async () => {
  ...
  try {
    ...
    try {
      ...
      console.log('No se encontraron datos previos en el archivo JSON. Se crearán nuevos datos.');
    ...
    for (const codigo of codigosLimpios) {
      ...
      try {
        ...
        if (objeto[dni]) {
          objeto[dni] = { ...objeto[dni], Expediente: codigo, nombre: nombreCompleto, Estado: estado, ultima_Revisión: fecha };
        } else {
          objeto[dni] = { Expediente: codigo, Nombre: nombreCompleto, Estado: estado, "Ultima Revisión": fecha };
        }
      } catch (error) {
